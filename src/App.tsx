...
export function App() {
  ...
  const pitchShift = tonalMode ? semitoneDistance(sourceRoot, targetRoot) : 0;
  ...
  async function translate() {
    ...
    try {
      ...
      const keyUnchanged = pitchShift === 0;

      if (bpmUnchanged && keyUnchanged) {
      ...
      const pitchText = pitchShift === 0 ? "KEY UNCHANGED" : `${pitchShift > 0 ? "+" : ""}${pitchShift} SEMITONES`;
  ...
  function exportTranslated() {
    if (!translatedBuffer) return;
    const keyPart = tonalMode ? `-${targetRoot.replace("#", "sharp")}` : "";
    downloadBlob(
      audioBufferToWav(translatedBuffer),
      `pattern-translator-${targetBpm}bpm${keyPart}.wav`,
    );
  }
    ...
    <main className="machineShell">
      <header className="machineHeader">
        <div>
          ...
          <div className="versionLine">DIRECT AUDIO TRANSLATOR // BUILD 0.5</div>
        </div>
        <div className="statusLed"><i /> LOCAL DSP</div>
      </header>

      <section className="module modeModule">
        <div className="moduleTitle">01 // WHAT ARE YOU TRANSLATING?</div>
          ...
          <>
            <div className="abDeck">
              <div className="abChannel originalChannel">
                ...
                <button className="abPlayButton" onClick={playOriginal}>
                  {playingOriginal ? <Pause size={15} /> : <Play size={15} />}
              ...
              <div className="fileReadout">
                <b>{file.name}</b>
                <span>{buffer?.duration.toFixed(1)} SEC // {buffer?.sampleRate.toLocaleString()} HZ // {rhythm?.beats.length ?? 0} BEATS</span>
              </div>

              <div className="abChannel rebuiltChannel">
                <span className="abLabel">B // TRANSLATED WAV</span>
                <button className="abPlayButton rebuilt" onClick={playTranslated} disabled={!translatedBuffer}>
                  {playingTranslated ? <Pause size={15} /> : <Play size={15} />}
                  {playingTranslated ? "STOP TRANSLATED" : "PLAY TRANSLATED"}
                </button>
              </div>
      ...
      <section className="module translateModule">
        <div className="moduleTitle">03 // SET THE NEW BPM {tonalMode ? "+ KEY" : ""}</div>
          ...
          {tonalMode && (
            <>
              <label className="keyControl">
                ...
                <select value={sourceRoot} onChange={(event) => { setSourceRoot(event.target.value); invalidateTranslation("SOURCE KEY CHANGED — TRANSLATE AGAIN"); }}>
              ...
              <label className="keyControl targetControl">
                ...
                <select value={targetRoot} onChange={(event) => { setTargetRoot(event.target.value); invalidateTranslation(); }}>
                  {ROOTS.map((root) => <option key={root}>{root}</option>)}
                </select>
              </label>
            </>
        ...
        <div className="actionRail">
          ...
          <button className="processButton" disabled={!file || busy} onClick={() => void translate()}>
            <Wand2 size={17} /> {busy ? "TRANSLATING…" : "TRANSLATE AUDIO"}
          </button>
        </div>

        {mode === "beat" && (
          <div className="midiWarning">
            FULL BEAT MODE transforms the original mix directly. BPM changes preserve pitch; key changes shift the whole mix. For the cleanest key change without pitching drums, use separated DRUMS + BASS + MELODY stems until automatic stem splitting lands.
          </div>
        )}
      </section>
      ...
      <section className="module patternModule">
        ...
        <div className="resultPanel">
          <div>
            ...
            <span>{translatedBuffer ? `${translatedBuffer.duration.toFixed(1)} SEC // ${targetBpm} BPM${tonalMode ? ` // ${targetRoot}` : ""}` : "SET TARGETS, THEN PRESS TRANSLATE AUDIO"}</span>
          </div>
          <button className="exportButton primaryExport" disabled={!translatedBuffer} onClick={exportTranslated}>
            <Download size={15} /> EXPORT TRANSLATED WAV
          </button>
        </div>
      </section>

      <footer className="machineFooter">
        <span><SlidersHorizontal size={12} /> DIRECT AUDIO TRANSFORM // NO MIDI RE-SYNTHESIS</span>
        <span>SAME SOURCE SOUND → NEW BPM / KEY</span>
      </footer>
    </main>
  );
}
