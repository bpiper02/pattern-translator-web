...
def run_separator(
    ...
    *,
    ...
    try:
        ...
        # Keep the HTTP-facing message concise; the full traceback is retained
        # in the backend terminal via LOGGER.exception above.
        raise RuntimeError(f"{type(exc).__name__}: {exc}") from exc
    finally:
        # Release model references promptly between local jobs.  This matters on
        # memory-constrained laptops where repeated splits can otherwise retain
        # large torch/onnx objects until a later GC cycle.
        separator = None
        gc.collect()
